  ...
  return (
    <View className="gap-1">
      <Pressable
        ...
        className={`h-10 w-10 items-center justify-center rounded-xl border border-line bg-cream ${index === 0 || disabled ? "opacity-40" : ""}`}
      >
        <ChevronUp size={18} color="#6f6151" />
      </Pressable>
      <Pressable
        ...
        className={`h-10 w-10 items-center justify-center rounded-xl border border-line bg-cream ${index === count - 1 || disabled ? "opacity-40" : ""}`}
      >
        <ChevronDown size={18} color="#6f6151" />
      </Pressable>
    </View>
  );
}
...
export default function TestStructureEditor({
  ...
  function shiftRow(from: number, to: number) {
    if (to < 0 || to >= rows.length || from === to) return;
    startOrderRef.current = rows.map((row) => row.question_id);
  ...
  return (
    <Card className="border-line">
      ...
      <Text className="mt-1 text-xs text-ink-soft">
        Edit the prompt, answer key, and marks. Use the arrows to change question order.
      </Text>
      ...
      <View className="mt-4">
        {rows.map((row, index) => (
          <View key={row.question_id} className="mb-2 rounded-xl border border-line bg-pen-wash/30 p-3">
